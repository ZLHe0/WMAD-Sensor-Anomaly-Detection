...
class FLOS_Trainer(BaseTrainer):
    ...
    def __init__(self, optimizer_name: str = 'adam', lr: float = 0.001, n_epochs: int = 150, lr_milestones: tuple = (),
        ...
        self.focal_loss_fn = FocalLoss(alpha=0.1, gamma=2)
    ...
    def adapt_and_test(self, finetune_dataset, testing_dataset, maml_learner, output_data_path, save_rep=False):
        ...
        with torch.no_grad():
            for data in test_loader:
                ...
                support_scores = torch.sigmoid(support_preds.reshape(-1))
...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        # Input feature should be 1-dimensional
        inputs = inputs.squeeze(1)
        ce_loss = F.cross_entropy(inputs, targets, reduction='none')
        pt = torch.exp(-ce_loss)
        alpha_factor = (1 - self.alpha) * targets + self.alpha * (1 - targets)
