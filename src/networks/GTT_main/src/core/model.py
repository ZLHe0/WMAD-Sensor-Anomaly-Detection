...
class GTT:
    ...
    # # ZH: get the embedding of the input data
    # def get_embeddings(self, time_series, window_size=48, batch_size=512):
    #     """
    #     Get the embeddings for each time window of a given size in a univariate time series.

    #     Parameters:
    #     time_series (ndarray): The univariate time series data.
    #     window_size (int): The size of the time windows.

    #     Returns:
    #     list: A list of embeddings, where each element corresponds to a time window of size `window_size`.
    #     """
    #     # Create a sliding window over the time series
    #     data = np.array(time_series, dtype=np.float32)
    ...
    #         sequence_stride=1,
    #         shuffle=False,
    #         batch_size=batch_size, # increase the batch size
    #     )

    #     window_embeddings = []
    #     for x in ds.as_numpy_iterator():
    #         # Ensure x has the correct shape (Batch_size, Time, Channel)
    #         x = tf.expand_dims(x, axis=-1)  # Add the channel dimension

    #         # Reshape and transpose the input sequence
    #         _, T, C = x.shape
    #         patch_num = self.configs.block_size // self.configs.patch_size
    #         x = tf.transpose(x, perm=[0, 2, 1])
    #         x = tf.reshape(x, [-1, T])

    #         # Get the embeddings from the encoder
    #         x_dec = self.estimator.encoder(x, patch_num, C)
    #         embeddings = x_dec[:, -1, :]  # B*C, n_embd
    #         embeddings_list = tf.unstack(embeddings, axis=0)

    #         window_embeddings = window_embeddings + embeddings_list

    #     return window_embeddings

    def get_embeddings(self, time_windows, window_size=48, batch_size=512):
        """
        Get the embeddings for a batch of time windows.

        Parameters:
        time_windows (ndarray): A batch of time windows with shape (n_shots, window_size, 1).
        window_size (int): The size of the time windows.
        batch_size (int): The batch size for processing through the GTT model.

        Returns:
        ndarray: An array of embeddings with shape (n_shots, embedding_dim).
        """
        # Reshape the time windows to (n_shots, window_size) and remove the channel dimension
        time_windows = np.squeeze(time_windows, axis=-1)  # Shape: (n_shots, window_size)

        # Create a TensorFlow dataset from the time windows
        ds = tf.data.Dataset.from_tensor_slices(time_windows)
        ds = ds.batch(batch_size)

        window_embeddings = []
        for batch in ds:
            # Ensure batch has the correct shape (Batch_size, Time, Channel)
            batch = tf.expand_dims(batch, axis=-1)  # Add the channel dimension
            
            # Reshape and transpose the input sequence
            _, T, C = batch.shape
            patch_num = self.configs.block_size // self.configs.patch_size
            batch = tf.transpose(batch, perm=[0, 2, 1])
            batch = tf.reshape(batch, [-1, T])

            # Get the embeddings from the encoder
            x_dec = self.estimator.encoder(batch, patch_num, C)
            embeddings = x_dec[:, -1, :]  # B*C, n_embd
        ...
        return window_embeddings.numpy()
